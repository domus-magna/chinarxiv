...
class TestCircuitBreaker:
    ...
    def test_persistent_error_threshold(self):
        ...
        assert self.service._consecutive_persistent == 1
        assert self.service._consecutive_transient == 0
        assert not self.service._circuit_open
        ...
        assert self.service._circuit_open
    ...
    def test_transient_error_threshold(self):
        ...
        assert self.service._consecutive_transient == 4
        assert self.service._consecutive_persistent == 0
    ...
    def test_error_classification_persistent(self):
        ...
        for code in persistent_codes:
            ...
            assert service._consecutive_persistent == 1, f"{code} should be classified as persistent"
    ...
    def test_error_classification_transient(self):
        ...
        for code in transient_codes:
            service = TranslationService(config=self.service.config)
            service._record_failure(code)
            assert service._consecutive_transient == 1, f"{code} should be classified as transient"
    ...
    def test_counter_reset_on_success(self):
        ...
        assert self.service._consecutive_persistent == 0  # Reset when switching to transient
        assert self.service._consecutive_transient == 1
    ...
    def test_alternating_errors_no_trigger(self):
        ...
        assert not self.service._circuit_open
        assert self.service._consecutive_transient == 2
        assert self.service._consecutive_persistent == 0
    ...
    @patch('src.services.translation_service.alert_critical')
    @patch('src.services.translation_service.log')
    ...
    def test_config_defaults(self):
        ...
        assert service.PERSISTENT_ERROR_THRESHOLD == 2
        assert service.TRANSIENT_ERROR_THRESHOLD == 5
    ...
    def test_custom_thresholds(self):
        ...
        assert service.PERSISTENT_ERROR_THRESHOLD == 3
        assert service.TRANSIENT_ERROR_THRESHOLD == 10
